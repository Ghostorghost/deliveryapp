...
class AgentSettingsScreen extends StatefulWidget {
  ...
  const AgentSettingsScreen({Key? key, this.onThemeChanged}) : super(key: key);
...
class _AgentSettingsScreenState extends State<AgentSettingsScreen> {
  ...
  Future<void> _loadSettingsFromHive() async {
    final box = await Hive.openBox('agentSettings');
    setState(() {
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              // Save password to Hive (not secure, demo only)
              Hive.openBox('agentSettings').then((box) {
